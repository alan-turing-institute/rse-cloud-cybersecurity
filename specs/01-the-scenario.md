...
3. A **virtual machine**, with a graphical desktop environment reachable over **RDP**, that comes with software pre-installed to (a) download data from the storage account and (b) run queries against the database. **VS Code** comes pre-installed and pre-configured as the *only* way the operator does both — connecting to the storage account and to the SQL database from inside the editor. No CLI tools (Azure CLI, `sqlcmd`) are installed as a backup path. **Google Chrome** is also pre-installed and set as the desktop's default web browser — the base Ubuntu image ships with no browser at all, and the operator needs one to reach the Azure Portal and other web resources from the RDP desktop.
...
- **The desktop needs a web browser, and the base Ubuntu image has none.** Google Chrome is installed via Google's official `apt` repository and set as the default browser (`x-www-browser` alternative, plus the desktop's `mimeapps.list` default handlers for `http`/`https`/`text/html`) so the operator has a working browser on first RDP login — e.g. to reach the Azure Portal — without an extra manual step. This is unrelated to the storage/database access path below, which deliberately stays VS Code-only.
- **VS Code is the only interface for storage/database access** — no Azure CLI, no `sqlcmd`, no other CLI tooling is installed on the VM. The operator connects to the storage account and the database exclusively from inside the editor. This is a deliberate simplification, not an oversight: one supported path is easier to keep working (and to demo) than a CLI fallback that would need its own upkeep alongside the VS Code path.
...
- `os_profile.custom_data` — base64-encoded cloud-init that installs:
  ...
  - **VS Code** (Microsoft's official `apt` repository — add the Microsoft signing key + repo, then `apt-get install code`) plus its `ms-mssql.mssql` and `ms-azuretools.vscode-azurestorage` extensions (`code --install-extension <id>`), installed non-interactively as part of the same cloud-init run. No other software is installed for storage/database access — see [Design principles](#design-principles) for why there's deliberately no CLI fallback.
  - **Google Chrome** (Google's official `apt` repository — same signing-key-plus-`sources` pattern used for VS Code — then `apt-get install google-chrome-stable`), set as the default browser via the `x-www-browser` `update-alternatives` entry and a `mimeapps.list` written for the admin user's desktop session (see [Design principles](#design-principles)).
...
- **Changing `custom_data` replaces the VM.** Azure silently ignores changes to `osProfile.customData` on an update — the cloud-init script only ever runs on first boot — so the resource is configured with `pulumi.ResourceOptions(replace_on_changes=["osProfile.customData"], delete_before_replace=True)`. Without this, editing the cloud-init template (e.g. adding a package) would report success but never actually apply on an existing VM. `delete_before_replace=True` makes Pulumi delete the old VM before creating the new one (rather than the default create-before-delete), since the replacement VM reuses the same NIC/computer name as the one being torn down.
...
- Storage account connection string/key: export the primary key or full connection string as a **secret** stack output the same way, to be pasted into the Azure Storage VS Code extension's "Attach Storage Account..." command — the only way the operator uses it, since no CLI tooling is installed on the VM.
