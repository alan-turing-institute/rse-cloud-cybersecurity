...
The current deployment (see [`00-the-scenario.md`](00-the-scenario.md)) has two credential-based access paths, both of which end up as secret stack outputs a human retrieves with `pulumi stack output --show-secrets` and pastes into VS Code by hand:
...
- **VS Code extensions move from SQL login to Azure AD auth — `ms-mssql.mssql` cannot actually ride the VM's managed identity, confirmed by reading the extension's own schema, not just its docs.** The Learn docs describe a "Microsoft Entra ID - Default" option that sounds like it could pick up a managed identity via a `DefaultAzureCredential`-style chain. But the extension's `package.json` (the actual `mssql.connections[].authenticationType` contribution schema, checked directly against the `microsoft/vscode-mssql` source) only defines four values: `"Integrated"`, `"SqlLogin"`, `"AzureMFA"`, and `"ActiveDirectoryServicePrincipal"` — there's no `"Default"`, and nothing managed-identity-flavored, in the profile format that can actually be pre-seeded into `settings.json`. "Microsoft Entra ID - Default" is UI-only presentation in the interactive Connection Dialog, not something expressible in a saved connection profile:
  - **`AzureMFA`** is the closest thing that exists — but it means interactive human Azure AD sign-in (a device-code/browser flow through the account the operator is signed into VS Code with), not the VM's managed identity riding silently in the background. It's what the pre-created profile uses (closest available fit for "no SQL login password"), but it does **not** deliver on "the VM authenticates via its own managed identity" from inside VS Code — the operator ends up authenticating as *themselves*, and would need the same RBAC roles granted to their own Azure AD account for it to actually work end to end (see [Open questions](#open-questions--assumptions-to-confirm-before-implementing)).
  - **`ActiveDirectoryServicePrincipal`** is wrong — it needs a client ID + client secret, which would just reintroduce a stored credential; the VM's managed identity doesn't have (or need) one. **`Integrated`** is Windows-only and irrelevant here.
  - The pre-created `mssql.connections` profile's `authenticationType` moves from `"SqlLogin"` to `"AzureMFA"`; `user`/`password`/`savePassword` are dropped, since `AzureMFA` doesn't use them.
  - **Because the extension genuinely cannot use the managed identity, `sqlcmd` (go-sqlcmd) with `--authentication-method ActiveDirectoryManagedIdentity` isn't just a defensive fallback — it's the only way this VM actually exercises the managed-identity path for the database at all.** It's [officially documented](https://learn.microsoft.com/en-us/sql/tools/sqlcmd/sqlcmd-authentication) and confirmed to work for a system-assigned identity with no extra arguments. The other two Microsoft SQL client tools that support an explicit "Managed Identity" auth mode don't fit this VM either: **SSMS** is Windows-only (this VM is Ubuntu), and **Azure Data Studio** has been retired/folded into the `ms-mssql.mssql` extension, so it's not a separate option any more.
  - **`sqlcmd` (go-sqlcmd) is installed on the VM itself, via cloud-init**, so this genuinely-working path is available without any extra setup — see [Virtual machine](#virtual-machine-infracomputepy) below. This deliberately revises [`00-the-scenario.md`'s "VS Code is the only interface, no CLI tooling" design principle](00-the-scenario.md#design-principles) for **database access specifically**, given that VS Code's `AzureMFA` auth type can't itself deliver a managed-identity-backed connection. Storage access is unaffected — it stays VS Code (or the `curl`-based manual check in [Verifying storage access from within the VM](#verifying-storage-access-from-within-the-vm-manual)) only, no CLI tooling added there.
...
- **Minimum-cost stays intact.** Managed identity and RBAC role assignments carry no additional Azure cost — this iteration changes the access *mechanism*, not the resource SKUs/tiers from `00-the-scenario.md`.
...
- `cloud-init` / the pre-created `mssql.connections` profile (`_vscode_settings_json` in `infra/compute.py`) changes as described above — `authenticationType` switches from `"SqlLogin"` to `"AzureMFA"` (the closest value the extension's schema actually supports — see [Design principles](#design-principles)), and the `user`/`password`/`savePassword` keys are dropped. The profile still needs `server` and `database`, which are unaffected.
- **New cloud-init step: install `sqlcmd` (go-sqlcmd)**, added to `infra/templates/vm-cloud-init.yaml.j2` alongside the existing VS Code/Chrome install steps — via the [official apt-based install instructions](https://learn.microsoft.com/en-us/sql/tools/sqlcmd/sqlcmd-download-install) for Debian/Ubuntu (Microsoft's `packages.microsoft.com` apt repository, matching the Ubuntu 22.04/jammy base image; package name `sqlcmd`). This is the database-access path described under [Design principles](#design-principles) — the only one that genuinely uses the VM's managed identity — always present on the VM from first boot, not something the operator has to install ad hoc. No configuration/profile is pre-seeded for it (unlike the mssql VS Code profile) since it's a command-line tool, not a GUI with a saved-connections file; the operator supplies `-S <server FQDN>`, `-d rse_demo_db`, and `--authentication-method ActiveDirectoryManagedIdentity` by hand when they use it.
...
Extending the existing `pulumi.runtime.set_mocks`-based suite (mirroring the module layout per [`00-the-scenario.md`](00-the-scenario.md#module-layout)):
...
- Per [`00-the-scenario.md`'s testing plan](00-the-scenario.md#testing-plan), the earlier scenario's tests deliberately didn't assert on identity/RBAC presence, since that posture was out of scope then — this iteration is where those tests get added, per that document's own note.
...
- Order of operations for a human running this (not Claude, per [`CLAUDE.md`](../CLAUDE.md#no-live-deployments-as-part-of-building-a-feature)):
  ...
  4. Re-attach the storage account in VS Code via Azure AD sign-in. For the database, either sign in through the pre-created `AzureMFA` mssql profile (as the operator's own Azure AD identity — needs the same data-plane roles granted to that identity, not just the VM's) or, to actually exercise the VM's managed identity, use the `sqlcmd` install already on the VM: `sqlcmd -S <server FQDN> -d rse_demo_db --authentication-method ActiveDirectoryManagedIdentity` — see [Design principles](#design-principles).
...
Per [`00-the-scenario.md`'s planned follow-up](00-the-scenario.md#planned-follow-up-later-iteration-not-part-of-this-scenario), this iteration is identity/RBAC only. Still deferred to later iterations:
...
2. **Resolved during implementation:** `ms-mssql.mssql` cannot ride the VM's managed identity at all — confirmed against the extension's actual `package.json` schema, which defines only `Integrated`/`SqlLogin`/`AzureMFA`/`ActiveDirectoryServicePrincipal` for `mssql.connections[].authenticationType` (see [Design principles](#design-principles)). What's still open: whether it's acceptable for the demo's VS Code path to mean the operator signs in as *themselves* via `AzureMFA` (needing the same RBAC roles granted to their own account, not just the VM's identity), or whether `sqlcmd`/`ActiveDirectoryManagedIdentity` on the VM should be treated as the primary database-access path instead of a fallback, given it's the only one that genuinely uses the managed identity.
3. **Does container-scoped `RoleAssignment` actually work for Storage Blob Data Reader as expected** (`scope=blob_container.id`)? Worth a final check against current Azure RBAC docs at implementation time — container-scoped assignments are supported for this role, but it's the kind of detail worth confirming rather than assuming, per this repo's existing convention (cf. `00-the-scenario.md`'s open questions on Public IP SKU retirement). [Verifying storage access from within the VM](#verifying-storage-access-from-within-the-vm-manual) above is exactly how to confirm this once deployed.
