...
**Consolidation note:** this document was written and verified against a version of the repository with no Application Firewall, no storage-account network ACL, and no VM managed identity/RBAC yet. By the time this plan reached `consolidation`, a separate follow-up (the storage-firewall consolidation) had independently given the VM's identity a broader, account-wide **Storage Blob Data Contributor** grant, so `az storage blob upload/download --auth-mode login` would work. That grant has been removed in favour of the read-only, container-scoped design below, kept unchanged from how it's described here — see [`specs/consolidating-managing-identities.md`](consolidating-managing-identities.md) for why, and for the accepted consequence that `az storage blob upload --auth-mode login` no longer works from the VM. Nothing else in this document needed correcting: its RBAC design, goals, and verification/troubleshooting procedures already describe and test exactly the grant that's kept.
...
This plan covers **identity only**, scoped to the VM's own access to storage. Database access via managed identity, RBAC for human/operator access to the resource group, VNet segmentation, Azure Bastion, NSG narrowing, and SSH-key auth all remain deferred to their own later iterations — see [Out of scope](#out-of-scope-still-deferred) below.
...
- No change to `os_profile` (VM login stays password-based — deferred to the SSH-key iteration) and no change to the existing `mssql.connections` cloud-init profile — database access via managed identity is a separate, later iteration.
