...
9. Allow `login.microsoftonline.com` and `management.azure.com` on port 443 (HTTPS) - Microsoft Entra ID and Azure Resource Manager, needed for `az login` and for the Azure CLI generally (see `README-Storage.md`'s "Accessing the storage account from the virtual machine").
