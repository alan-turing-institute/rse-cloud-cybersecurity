...
**Consolidation note (managed identity):** the VM's actual storage access (see `README-Storage.md`) uses the VM's own system-assigned managed identity, granted **Storage Blob Data Reader** - read-only, scoped to the `rse-demo-container` container only, not the whole storage account - directly by this Pulumi program (`infra/compute.py`'s `storage_blob_data_reader_role_assignment`) rather than by a manual role assignment for a human's account. Two paths use this identity, both via the VM's Instance Metadata Service (`169.254.169.254`), which - like the platform DNS server - is exempt from the `0.0.0.0/0` route and reachable regardless of this firewall, so rule 9 isn't actually needed for either: `az login --identity` (for `az storage blob download --auth-mode login` - uploading no longer works from the VM, since the grant is read-only) and BlobFuse2's `mode: msi` (for the read-only filesystem mount at `/mnt/rse-demo-container`). Rule 9 is kept as-is because `az login --use-device-code` (signing in as a human, for anything other than the storage workflow) is still a legitimate, supported thing to do on this VM.
