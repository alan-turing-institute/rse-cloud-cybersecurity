...
7. Allow `*.blob.core.windows.net` on port 443 (HTTPS) - the Storage Account public endpoint.
8. Allow `login.microsoftonline.com`, `login.windows.net`, `*.login.microsoft.com`, and `management.azure.com` on port 443 (HTTPS) - Microsoft Entra ID's token endpoints and Azure Resource Manager, needed for `az login --use-device-code` and for the Azure CLI generally when signed in as a *user* (see `README-Storage.md`'s "Accessing the storage account from the virtual machine").

**Consolidation note (storage):** once `05-storage` lands (see `README-Storage.md`), the virtual machine's own subnet gets a `Microsoft.Storage` service endpoint, so its storage traffic takes a more specific route than this firewall's `0.0.0.0/0` UDR and reaches the Storage Account directly over the Azure backbone. Rule 7 above no longer sees any real VM traffic as a result - it's left in place as harmless defence in depth, in case the service endpoint is ever removed. Rule 8 was added alongside it once it turned out the Azure CLI wasn't installed on the VM at all - it's now installed via cloud-init (see `infra/templates/vm-cloud-init.yaml.j2` and `README-Storage.md`'s "Accessing the storage account from the virtual machine").

**Consolidation note (`az login`):** rule 8's four hosts are deliberately just the token-protocol endpoints, not the Microsoft sign-in *page's* CDN assets (e.g. `aadcdn.msftauth.net`) - those aren't a small, fixed set worth chasing, same reasoning as the VS Code Marketplace gap below. `az login --use-device-code` never needs the sign-in page to render on the VM at all (the human completes it on a separate device), so it works with just these four hosts; plain `az login` doesn't, and isn't supported here - see `README-Storage.md`.

**Consolidation note (managed identity):** the VM's actual storage access (see `README-Storage.md`) uses the VM's own system-assigned managed identity, granted **Storage Blob Data Reader** - read-only, scoped to the `rse-demo-container` container only, not the whole storage account - directly by this Pulumi program (`infra/compute.py`'s `storage_blob_data_reader_role_assignment`) rather than by a manual role assignment for a human's account. Two paths use this identity, both via the VM's Instance Metadata Service (`169.254.169.254`), which - like the platform DNS server - is exempt from the `0.0.0.0/0` route and reachable regardless of this firewall, so rule 8 isn't actually needed for either: `az login --identity` (for `az storage blob download --auth-mode login` - uploading no longer works from the VM, since the grant is read-only) and BlobFuse2's `mode: msi` (for the read-only filesystem mount at `/mnt/rse-demo-container`). Rule 8 is kept as-is because `az login --use-device-code` (signing in as a human, for anything other than the storage workflow) is still a legitimate, supported thing to do on this VM.

One known gap: the VS Code Marketplace extension the cloud-init script also runs (`ms-azuretools.vscode-azurestorage`) reaches additional Microsoft CDN endpoints not enumerated here (e.g. `marketplace.visualstudio.com`, `*.gallerycdn.vsassets.io`) - these weren't added because that endpoint set isn't officially pinned down and can change; if `pulumi up` shows cloud-init failing on the extension install step specifically, check the VM's `/var/log/cloud-init-output.log` for the blocked host and add it here.
