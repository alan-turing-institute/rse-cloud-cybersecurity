...
6. Select (or create) a stack:
   ...
   `db-admin-username` and `vm-admin-username` are configurable (see `Pulumi.dev.yaml` for their defaults). The VM and PostgreSQL admin passwords are generated by Pulumi and exported as secret stack outputs — retrieve them after deploying with `pulumi stack output vm_admin_password --show-secrets` and `pulumi stack output postgres_admin_password --show-secrets`.
...
- `infra/` — infrastructure resource definitions, split by concern and re-exported from `infra/__init__.py` for `__main__.py` and the test suite:
  ...
  - `naming.py` — a random suffix shared by globally-unique resource names (storage account, PostgreSQL server)
  - `networking.py` — VNet, subnet, public IP, and NIC for the virtual machine
  - `storage.py` — the storage account and its blob container
  - `database.py` — the PostgreSQL Flexible Server, firewall rule, and database
...
See [`specs/01-the-scenario.md`](specs/01-the-scenario.md) for the design behind the current infrastructure — a storage account, a PostgreSQL Flexible Server, and a Linux VM able to reach both, all kept at minimum cost. This is the initial iteration: security hardening (private networking, managed identity, RBAC) is intentionally deferred to a later iteration, so the storage account, database, and VM are all reachable over the public internet.
